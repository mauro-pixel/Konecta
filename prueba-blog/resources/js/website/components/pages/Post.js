...
class Post extends React.Component {
    ...
    componentDidMount() {
        ...
        PostApi.getById(this.props.match.params.id).then(response =>
            this.setState({
                    ...
                    <div id="main" className="eight columns">
                        <img
                            src={
                                ...
                                "assets/website/images/ajax-loader.gif"
                            }
                            style={{
                                display:
                            ...
                            <article className="entry">
                                <header className="entry-header">
                                    ...
                                    <div className="entry-meta">
                                        <ul>
                                            <li>
                                                {this.state.post.date_formatted}
                                            </li>
                                            <span className="meta-sep">•</span>
                                            <li>
                                                <Link
                                                    to={`/category/${this.state.post.category.id}/${this.state.post.category.slug}`}
                                                    title=""
                                                    rel="category tag"
                                                >
                                                    {
                                ...
                                {this.state.post.hasOwnProperty("tags") &&
                                this.state.post.tags.length > 0 ? (
                                    <p className="tags">
                                        <span>Etiquetado en </span>:
                                        {this.state.post.tags.map(
                                            (tag, index) => (
                                                <span>
                                                    <Link
                                                        key={tag.id}
                        ...
                        <div id="comments">
                            {this.state.post.hasOwnProperty(
                                "approved_comments"
                            ) &&
                            this.state.post.approved_comments.length > 0 ? (
                                <div>
                                    <h3>
                                        {
